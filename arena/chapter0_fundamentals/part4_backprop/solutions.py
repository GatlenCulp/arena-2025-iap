...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    dims_to_sum = tuple(
        [
            i
            for i, (o, b) in enumerate(zip(original.shape, broadcasted.shape, strict=False))
            if o == 1 and b > 1
        ],
    )
...
def sum_back(grad_out: Arr, out: Arr, x: Arr, dim=None, keepdim=False):
    ...
    # Step (1): if keepdim=False, then we need to add back in dims, so grad_out and x have same number of dims
    if not keepdim:
        if dim is None:
...
def test(model: MLP, test_loader: DataLoader, test_accuracy_list: list | None = None):
    ...
    test_loss /= n_data
    print(
        f"Test set:  Avg loss: {test_loss:.3f}, Accuracy: {test_accuracy}/{n_data} ({test_accuracy / n_data:.1%})",
    )
