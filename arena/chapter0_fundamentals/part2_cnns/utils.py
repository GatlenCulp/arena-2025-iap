...
def print_param_count(
    *models,
    ...
    if len(models) > 1:
        ...
        param_shapes_sorted = [
            sorted(df[f"shape_{i}"].values.tolist()) for i in range(1, len(models) + 1)
        ]
        if all([param_shapes[0] == param_shapes[i] for i in range(1, len(param_shapes))]):
            ...
            [
                param_shapes_sorted[0] == param_shapes_sorted[i]
                for i in range(1, len(param_shapes_sorted))
            ],
        ):
            print("Models have the same parameter shapes, but they're in a different order.")
        else:
            print(
                "Mismatch detected: models do not have the exact same parameter shapes (even up to permutations).",
            )
