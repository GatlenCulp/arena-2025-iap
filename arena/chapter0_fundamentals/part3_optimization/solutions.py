...
class SGD:
    ...
    def step(self) -> None:
        ...
        for b, theta in zip(self.b, self.params, strict=False):
            g = theta.grad
            if self.lmda != 0:
                g = (
                    g + self.lmda * theta
...
class RMSprop:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for theta, b, v in zip(self.params, self.b, self.v, strict=False):
            g = theta.grad
            if self.lmda != 0:
                g = g + self.lmda * theta
            v.copy_(
...
class Adam:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for theta, m, v in zip(self.params, self.m, self.v, strict=False):
            g = theta.grad
            if self.lmda != 0:
                g = g + self.lmda * theta
...
class SGD:
    ...
    def step(self) -> None:
        ...
        for param_group in self.param_groups:
            ...
            for b, theta in zip(param_group["b"], param_group["params"], strict=False):
                g = theta.grad
                if lmda != 0:
                    g = (
                        g + lmda * theta
...
def test_resnet_on_random_input(model: ResNet34, n_inputs: int = 3, seed: int | None = 42):
    ...
    for img, label, prob in zip(imgs, classes, probs, strict=False):
